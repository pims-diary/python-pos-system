...
class ManageProducts:
    ...
    def get_search_type(self):
        while True:
            ...
            if search_criteria == '1':
                ...
                return Resources.FEATURE_ACCESS_LEVEL_TEXT, self.user, self.product
            elif search_criteria == '2':
                print("")
                self.get_product_to_search('name')
                return Resources.FEATURE_ACCESS_LEVEL_TEXT, self.user, self.product
            elif search_criteria == '0':
                print("Exiting Product Search")
                print("")
                return Resources.FEATURE_ACCESS_LEVEL_TEXT, self.user, self.product
            else:
                print("")
                AppDesigns.print_error("Invalid Input")
                print("")
    ...
    def get_product_to_search(self, by):
        ...
        while True:
            print("")
            AppDesigns.print_special("Enter a Product " + search_criteria + ".")
            value = AppDesigns.user_input("Enter here: ")

            self.search_product(by, value)

            if not isinstance(self.product, Product):
                print("")
                AppDesigns.print_error("This product was not found.")
                print("")
            elif len(self.product.product) != 0:
                print("")
                AppDesigns.print_special("PRODUCT DETAILS")
                ...
                print("")
                AppDesigns.print_error("Oops. Something wrong with this request. Please try again.")
                print("")
    ...
    def list_all_products(self):
        print("")
        AppDesigns.print_special("LIST OF ALL PRODUCTS AVAILABLE IN STORE")
        for index in self.products:
            print("")
            self.product = Product(self.products[int(index)])
            print("Product ID: " + self.product.id)
